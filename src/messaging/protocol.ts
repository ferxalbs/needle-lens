...
export function isVisibleCandidate(value: unknown): value is VisibleCandidate {
  ...
  return (
    ...
    typeof record.text === 'string' && record.text.length > 0 && record.text.length <= 20_000 &&
    (record.author === undefined || typeof record.author === 'string') &&
    (record.canonicalUrl === undefined || typeof record.canonicalUrl === 'string') &&
    (record.viewportState === 'visible' || record.viewportState === 'near-viewport') &&
    typeof record.viewportIndex === 'number' && Number.isInteger(record.viewportIndex)
  );
}
